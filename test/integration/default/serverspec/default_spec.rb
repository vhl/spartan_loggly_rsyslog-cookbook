...
def rsyslog_version7?
  os[:family] == 'ubuntu' && os[:release].to_f < 16.04 || os[:family] == 'redhat' && os[:release].to_f >= 7.0
end
...
def rsyslog_version8_and_above?
...
describe file('/etc/rsyslog.d/20-input-module-file.conf') do
  it { should be_file }

  if rsyslog_version7_and_above?
    expected_imfile_conf = <<-EOS.gsub(/^      /, '')
  ...
  its(:content) { should eq expected_imfile_conf }
end

describe file('/etc/rsyslog.d/99-files.conf') do
  it { should be_file }

  if rsyslog_version8_and_above?
...
describe file('/etc/rsyslog.d/21-nginx.conf') do
  ...
  if rsyslog_version8_and_above?
    expected_apps_conf = <<-EOS.gsub(/^      /, '')
      ...
      input(type="imfile"
            File="/tmp/nginx-access.log"
            Tag="custom-nginx-access"
        )
      input(type="imfile"
            File="/tmp/nginx-ssl-access.log"
            Tag="custom-nginx-access"
        )
      input(type="imfile"
            File="/tmp/nginx-error.log"
            Tag="nginx-error-log"
        )
    ...
    expected_apps_conf = <<-EOS.gsub(/^      /, '')
      ...
      input(type="imfile"
            File="/tmp/nginx-ssl-access.log"
            Tag="custom-nginx-access"
            Statefile="/tmp/nginx-ssl-access.log.rsyslog_state"
        )
      input(type="imfile"
            File="/tmp/nginx-error.log"
            Tag="nginx-error-log"
            Statefile="/tmp/nginx-error.log.rsyslog_state"
        )
