...
describe 'spartan_loggly_rsyslog::default' do
  ...
  let(:input_module_conf) { '/etc/rsyslog.d/20-input-module-file.conf' }
  ...
  it 'defines an input module template that does nothing unless notified' do
    expect(chef_run.template(input_module_conf)).to do_nothing
  end
